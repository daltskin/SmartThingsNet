...
namespace SmartThingsNet.Model
{
    ...
    /// PushButton
    /// </summary>
    [DataContract(Name = "pushButton")]
    public partial class PushButton : IEquatable<PushButton>, IValidatableObject
    {
        /// <summary>
        ...
        public PushButton(string command = default(string), string argument = default(string), string argumentType = default(string))
        {
            // to ensure "command" is required (not null)
            if (command == null) {
                throw new ArgumentNullException("command is a required property for PushButton and cannot be null");
            }
        ...
        {
            return this.Equals(input as PushButton);
        }

        /// <summary>
        /// Returns true if PushButton instances are equal
        /// </summary>
        /// <param name="input">Instance of PushButton to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(PushButton input)
        {
            if (input == null)
            {
                return false;
