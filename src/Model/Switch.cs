...
namespace SmartThingsNet.Model
{
    ...
    /// Switch
    /// </summary>
    [DataContract(Name = "_switch")]
    public partial class Switch : IEquatable<Switch>, IValidatableObject
    {
        /// <summary>
        ...
        public Switch(SwitchCommandCommand command = default(SwitchCommandCommand), SwitchStateState state = default(SwitchStateState))
        {
            // to ensure "command" is required (not null)
            if (command == null) {
                throw new ArgumentNullException("command is a required property for Switch and cannot be null");
            }
        ...
        {
            return this.Equals(input as Switch);
        }

        /// <summary>
        /// Returns true if Switch instances are equal
        /// </summary>
        /// <param name="input">Instance of Switch to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(Switch input)
        {
            if (input == null)
            {
                return false;
