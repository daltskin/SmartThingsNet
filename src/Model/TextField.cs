...
{
    /// <summary>
    /// This display type creates textfield that the user enters a new value and that the new value is passed as an argument to the command
    /// </summary>
    ...
    public partial class TextField : IEquatable<TextField>, IValidatableObject
    {
        ...
        public TextField(string command = default(string), string argumentType = default(string), string value = default(string), string valueType = default(string), List<double> range = default(List<double>))
        {
            // to ensure "command" is required (not null)
            if (command == null) {
                throw new ArgumentNullException("command is a required property for TextField and cannot be null");
            }
