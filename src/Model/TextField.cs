...
namespace SmartThingsNet.Model
{
    ...
    public partial class TextField :  IEquatable<TextField>, IValidatableObject
    {
        ...
        public TextField(string command = default(string), string value = default(string), List<double> range = default(List<double>))
        {
            // to ensure "command" is required (not null)
            this.Command = command ?? throw new ArgumentNullException("command is a required property for TextField and cannot be null");
            this.Value = value;
            this.Range = range;
        }
