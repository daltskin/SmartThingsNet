...
namespace SmartThingsNet.Model
{
    ...
    public partial class NumberField :  IEquatable<NumberField>, IValidatableObject
    {
        ...
        public NumberField(string value = default(string), string unit = default(string), string command = default(string), List<double> range = default(List<double>))
        {
            // to ensure "command" is required (not null)
            this.Command = command ?? throw new ArgumentNullException("command is a required property for NumberField and cannot be null");
            this.Value = value;
            this.Unit = unit;
            this.Range = range;
        }
